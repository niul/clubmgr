<%@ include file="/WEB-INF/views/taglibs.jsp"%>
      ...
      <h1><fmt:message key="news.title"/></h1>
	  ...
	  <article>
		...
		${post.excerpt}
		<p>
		<a href='<c:url value="/post/${post.id}"/>' class="button icon fa-file-text-o"><fmt:message key="news.posted.more"/></a>
	  </article>
	  
	  </c:forEach>
	  <c:if test="${previous >= 0}">
	  <a href="news.html?start=${previous}" class="button icon fa-files-o"><fmt:message key="news.previous"/></a>
	  </c:if> 
	  <c:if test="${next ne '0'}">
	  <a href="news.html?start=${next}" class="button icon fa-files-o"><fmt:message key="news.next"/></a>
	  </c:if>