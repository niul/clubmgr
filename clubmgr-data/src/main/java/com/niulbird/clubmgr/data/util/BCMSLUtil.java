...
public class BCMSLUtil extends BaseUtil {
    private static final Logger log = LoggerFactory.getLogger(BCMSLUtil.class);
    
    private final static String TIME_FORMAT = "EEE MM/d/yyyy h:mma";
    private final static String DATE_FORMAT = "EEE MM/d/yyyy h:mma";
    
	private static final String BCMSL_URI = "https://bcmsl.ca/";
    
    public BCMSLUtil(Properties props) {
    	this.props = props;
    }
					...
					if (headerRow != null && headerRow.size() > 1
						...
						for (int i = 1; i < rows.size(); i++) {
							...
							if (columns.size() > 1) {
								...
								if (fieldLink.size() > 0) {
									String bcmslFieldLink = BCMSL_URI + fieldLink.get(0).attr("href");
									String fieldMapUri = null;
									try {
										Document fieldDoc = Jsoup.connect(bcmslFieldLink).get();
